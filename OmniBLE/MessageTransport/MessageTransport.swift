...
public struct MessageTransportState: Equatable, RawRepresentable {
    ...
    public var msgNum: UInt8 // 7-bit BLE message #
    public var messageNumber: UInt8 // 5-bit Omnipod Message #

    public var ltk: Data
    public var cbuuid: CBUUID // ZZZ does this need to be an array?
    
    init(messageNumber: Int, msgNum: UIint8, ltk: Data, cbuuid: CBUUID) {
    ...
    public init?(rawValue: RawValue) {
        guard
            let messageNumber = rawValue["messageNumber"] as? UInt8
            let msgNum = rawValue["msgNum"] as? UInt8
            let ltk = rawValue["ltk"] as? Data          // ZZZ will need work
            let cbuuid: = rawValue["ltk"] as? Data      // ZZZ will need work
            else {
                return nil
        }
        self.messageNumber = messageNumber
...
class OmniBLEMessageTransport: MessageTransport {
    
    private let session: CommandSession
    
    private let log = OSLog(category: "OmniBLEMessageTransport")
    ...
    private(set) var messageNumber: UInt8 {
        get {
    ...
    private(set) var msgNum: UInt8 {
        get {
            return state.msgNum
        }
        set {
            state.msgNum = newValue
        }
    }
    ...
    private func incrementMsgNum(_ count: Int = 1) {
        msgNum = (msgNum + count) & 0b01111111
    }
    
    
    private func incrementMessageNumber(_ count: Int = 1) {
        messageNumber = (messageNumber + count) & 0b00001111
    }

/* ZZZ not needed funcs for OmniBLE?
...
// ZZZ need to rework this func for OmniBLE!
    /// Handles sending an Omnipod Message to the BLE Pod and returns its response message 
    ///
    /// - Parameters:
